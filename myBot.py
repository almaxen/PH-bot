...
def callback_worker(call):
    if call.message:
        if call.data == "b1":
            bot.send_message(call.message.chat.id, text='Слушаю', reply_markup=None)
        elif call.data == "b2":
            bot.send_message(call.message.chat.id, text='Стартуем', reply_markup=None)
        elif call.data == "b3":
            bot.send_message(call.message.chat.id, text='* BOOM *', reply_markup=None)
        elif call.data == "b4":
            bot.send_message(call.message.chat.id, text='К Оле Гончаровой', reply_markup=None)
        elif call.data == "b5":
            bot.send_message(call.message.chat.id, text='У нас так не принято', reply_markup=None)
